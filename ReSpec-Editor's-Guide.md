...
That `index.html` file is a dump of the `source.html` file’s DOM, generated by processing the source with ReSpec — that is, exactly the same output you get by manually using the Ctrl+Shift+Alt+S key combination within the document itself and picking "Save as HTML (Source)" to save the output.

You can use additional `-F` flags with `curl` to specify particular ReSpec configuration options; for example:

```
curl -F "file=@source.html" -F "publishDate=2020-02-21" https://labs.w3.org/spec-generator/ > index.html
```
